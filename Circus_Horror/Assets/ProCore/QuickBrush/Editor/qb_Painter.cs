...
//	private qb_ObjectContainer		container; 
	...
	void OnEnable()
	{
		...
		string guiPath = AssetDatabase.GetAssetPath(MonoScript.FromScriptableObject(this));//Application.GetAssetPath(this);			

		if(guiPath != string.Empty)
		{
			//Trim the directory and get the resources folder
			guiPath = guiPath.Replace("Editor/qb_Painter.cs","Resources/Skin/");
		}
		...
		savedBrushIcon			= Resources.LoadAssetAtPath(guiPath + "qb_SavedBrushIcon.tga", typeof(Texture2D)) as Texture2D;
		
		saveBrushIcon			= Resources.LoadAssetAtPath(guiPath + "qb_SaveBrushIcon.tga", typeof(Texture2D)) as Texture2D;
		saveBrushIcon_hover		= Resources.LoadAssetAtPath(guiPath + "qb_SaveBrushIcon_hover.tga", typeof(Texture2D)) as Texture2D;
		clearBrushIcon			= Resources.LoadAssetAtPath(guiPath + "qb_ClearBrushIcon.tga", typeof(Texture2D)) as Texture2D;
		clearBrushIcon_hover	= Resources.LoadAssetAtPath(guiPath + "qb_ClearBrushIcon_hover.tga", typeof(Texture2D)) as Texture2D;

		resetSliderIcon			= Resources.LoadAssetAtPath(guiPath + "qb_ResetSliderIcon.tga", typeof(Texture2D)) as Texture2D;
		resetSliderIcon_hover	= Resources.LoadAssetAtPath(guiPath + "qb_ResetSliderIcon_hover.tga", typeof(Texture2D)) as Texture2D;
	...
	void EnableMenu()
	{
		...
		brushStateArray = GetSlotStates();
		brushNameArray = GetSlotNames();
	}
...
#region Rotation Settings Vars
	...
	[SerializeField] private bool		flipNormalAlign =		false;
	[SerializeField] private bool		flipStrokeAlign =		false;	
	//Offset
//	[SerializeField] private Vector3	rotationRange = 		Vector3.zero;
	[SerializeField] private Vector3	rotationRangeMin =		Vector3.zero;
	[SerializeField] private Vector3	rotationRangeMax =		Vector3.zero;
#endregion
...
#region Sorting Vars  
	...
	[SerializeField] private string		brushName =				string.Empty;
	//private string newBrushName = string.Empty;
#region Live Vars
	//Painting
...
//			    EditorUtility.SetDirty(prefabGroup);
		...
		#region Layers		
            ...
            EditorGUILayout.BeginVertical(menuBlockStyle,GUILayout.Width(260));
				...
				paintToLayer = EditorGUILayout.Toggle("Paint to Layer", paintToLayer,EditorStyles.toggle); DoTipCheck("Restrict painting to specific layers?");
				//A dropdown where the user can check off which layers to paint to
				EditorGUI.BeginDisabledGroup(!paintToLayer);
				layerIndex = EditorGUILayout.MaskField("Choose Layers",layerIndex,GetLayerList(),EditorStyles.layerMaskField); DoTipCheck("Choose the layers to paint onto");
				layersMasked = layerIndex;

				EditorGUI.EndDisabledGroup();
				
				paintToSelection = EditorGUILayout.Toggle("Restrict to Selection", paintToSelection); DoTipCheck("Restrinct painting to selected objects in the scene - stacks with Layer Settings");
			...
			if(rotationFoldout == true)
			{
                ...
                EditorGUILayout.BeginVertical(menuBlockStyle,GUILayout.Width(260));
					EditorGUILayout.BeginHorizontal();
						...
						alignToNormal = EditorGUILayout.Toggle(alignToNormal,GUILayout.Width(14)); DoTipCheck("Placed objects orient based on the surface normals of the painting surface");
							GUILayout.Space(42);
						EditorGUI.BeginDisabledGroup(!alignToNormal);
						EditorGUILayout.LabelField("Flip",GUILayout.Width(40)); DoTipCheck("Flip object's up axis along the surface");
							flipNormalAlign = EditorGUILayout.Toggle(flipNormalAlign,GUILayout.Width(14)); DoTipCheck("Flip object's up axis along the surface");		
						EditorGUI.EndDisabledGroup();
					EditorGUILayout.EndHorizontal();
					...
					EditorGUILayout.BeginHorizontal();
						...
						alignToStroke = EditorGUILayout.Toggle(alignToStroke,GUILayout.Width(14)); DoTipCheck("Placed objects face in the direction of painting");
							GUILayout.Space(42);
						EditorGUI.BeginDisabledGroup(!alignToStroke);
							EditorGUILayout.LabelField("Flip",GUILayout.Width(40)); DoTipCheck("Flip object's forward axis along the stroke");
							flipStrokeAlign = EditorGUILayout.Toggle(flipStrokeAlign,GUILayout.Width(14)); DoTipCheck("Flip object's forward axis along the stroke");
						EditorGUI.EndDisabledGroup();
					EditorGUILayout.EndHorizontal();
                ...
                EditorGUILayout.BeginVertical(menuBlockStyle,GUILayout.Width(260));
						...
						EditorGUILayout.BeginHorizontal();
							...
							//rotationRange.x = EditorGUILayout.Slider(rotationRange.x,0f,180f); DoTipCheck("Upper limit (in degrees) to randomly offset object rotation around the X axis");
								...
								{	rotationRangeMin.x = 0f;
									rotationRangeMax.x = 0f;
								} DoTipCheck("Reset slider to 0");
							EditorGUILayout.LabelField(rotationRangeMin.x.ToString("000"),GUILayout.Width(30)); DoTipCheck("Limits (in degrees) to randomly offset object rotation around the X axis");
								EditorGUILayout.MinMaxSlider(ref rotationRangeMin.x,ref rotationRangeMax.x,-180f,180); DoTipCheck("Limits (in degrees) to randomly offset object rotation around the X axis");
								rotationRangeMin.x = (float)System.Math.Round(rotationRangeMin.x,0);
								rotationRangeMax.x = (float)System.Math.Round(rotationRangeMax.x,0);
							EditorGUILayout.LabelField(rotationRangeMax.x.ToString("000"),GUILayout.Width(30)); DoTipCheck("Limits (in degrees) to randomly offset object rotation around the X axis");
						EditorGUILayout.EndHorizontal();
						...
						EditorGUILayout.BeginHorizontal();
							...
							//rotationRange.y = EditorGUILayout.Slider(rotationRange.y,0f,180f); DoTipCheck("Upper limit (in degrees) to randomly offset object rotation around the Y axis");
								if(GUILayout.Button("",picButtonStyle,GUILayout.Width(16),GUILayout.Height(16)))
								{	rotationRangeMin.y = 0f;
									rotationRangeMax.y = 0f;
								} DoTipCheck("Reset slider to 0");
							EditorGUILayout.LabelField(rotationRangeMin.y.ToString("000"),GUILayout.Width(30)); DoTipCheck("Limits (in degrees) to randomly offset object rotation around the Y axis");
								EditorGUILayout.MinMaxSlider(ref rotationRangeMin.y,ref rotationRangeMax.y,-180f,180); DoTipCheck("Limits (in degrees) to randomly offset object rotation around the Y axis");
								rotationRangeMin.y = (float)System.Math.Round(rotationRangeMin.y,0);
								rotationRangeMax.y = (float)System.Math.Round(rotationRangeMax.y,0);
							EditorGUILayout.LabelField(rotationRangeMax.y.ToString("000"),GUILayout.Width(30)); DoTipCheck("Limits (in degrees) to randomly offset object rotation around the Y axis");
						EditorGUILayout.EndHorizontal();
						...
						EditorGUILayout.BeginHorizontal();
							...
							//rotationRange.z = EditorGUILayout.Slider(rotationRange.z,0f,180f); DoTipCheck("Upper limit (in degrees) to randomly offset object rotation around the Z axis");
								if(GUILayout.Button("",picButtonStyle,GUILayout.Width(16),GUILayout.Height(16)))
								{	rotationRangeMin.z = 0f;
									rotationRangeMax.z = 0f;
								} DoTipCheck("Reset slider to 0");
							EditorGUILayout.LabelField(rotationRangeMin.z.ToString("000"),GUILayout.Width(30)); DoTipCheck("Limits (in degrees) to randomly offset object rotation around the Z axis");
								EditorGUILayout.MinMaxSlider(ref rotationRangeMin.z,ref rotationRangeMax.z,-180f,180); DoTipCheck("Limits (in degrees) to randomly offset object rotation around the Z axis");
								rotationRangeMin.z = (float)System.Math.Round(rotationRangeMin.z,0);
								rotationRangeMax.z = (float)System.Math.Round(rotationRangeMax.z,0);
							EditorGUILayout.LabelField(rotationRangeMax.z.ToString("000"),GUILayout.Width(30)); DoTipCheck("Limits (in degrees) to randomly offset object rotation around the Z axis");
		...
		{	
			EditorGUILayout.BeginVertical(menuBlockStyle,GUILayout.Width(260));
				...
				EditorGUILayout.BeginHorizontal();
					...
					positionOffset.x = EditorGUILayout.FloatField(positionOffset.x); DoTipCheck("Offset final placement position along local X axis");
				EditorGUILayout.EndHorizontal();
				EditorGUILayout.BeginHorizontal();	
					...
					positionOffset.y = EditorGUILayout.FloatField(positionOffset.y); DoTipCheck("Offset final placement position along local Y axis");
				EditorGUILayout.EndHorizontal();
				EditorGUILayout.BeginHorizontal();
					EditorGUILayout.LabelField("Offset Z", GUILayout.Width(130)); DoTipCheck("Offset final placement position along local Z axis");
					positionOffset.z = EditorGUILayout.FloatField(positionOffset.z); DoTipCheck("Offset final position placement along local Z axis");
				EditorGUILayout.EndHorizontal();
			...
			if(scaleFoldout == true)
			{
				...
				EditorGUI.BeginDisabledGroup(!scaleUniform);
					...
					EditorGUILayout.BeginHorizontal(GUILayout.Width(260));
                        ...
                        EditorGUILayout.BeginVertical(menuBlockStyle,GUILayout.Width(78));
							...
							EditorGUILayout.LabelField(scaleRandMinUniform.ToString("0.00") + " to " + scaleRandMaxUniform.ToString("0.00"),GUILayout.Width(78)); DoTipCheck("Random Scaling Range Split Slider (Min/Max)");
							...
							scaleRandMinUniform = (float)System.Math.Round(scaleRandMinUniform,2);
							scaleRandMaxUniform = (float)System.Math.Round(scaleRandMaxUniform,2);			
				...
				EditorGUI.EndDisabledGroup();
                        ...
                        EditorGUILayout.BeginVertical(menuBlockStyle, GUILayout.Width(170), GUILayout.MaxWidth(170));
							...
							scaleMin = (float)System.Math.Round(scaleMin,2);
							scaleMax = (float)System.Math.Round(scaleMax,2);
						EditorGUILayout.EndVertical();
				...
				EditorGUI.BeginDisabledGroup(scaleUniform);
					...
					EditorGUILayout.BeginHorizontal();
                        ...
                        EditorGUILayout.BeginVertical(menuBlockStyle);
							...
							EditorGUILayout.LabelField(scaleRandMin.x.ToString("0.00") + " to " + scaleRandMax.x.ToString("0.00"),GUILayout.Width(76)); DoTipCheck("X Axis Random Scaling Range Split Slider (Min/Max)");
							EditorGUILayout.MinMaxSlider(ref scaleRandMin.x,ref scaleRandMax.x,scaleMin,scaleMax); DoTipCheck("X Axis Random Scaling Range Split Slider (Min/Max)");
							
							scaleRandMin.x = (float)System.Math.Round(scaleRandMin.x,2);
							scaleRandMax.x = (float)System.Math.Round(scaleRandMax.x,2);
							scaleRandMin.x = Mathf.Clamp(scaleRandMin.x,scaleMin,scaleMax);
                        ...
                        EditorGUILayout.BeginVertical(menuBlockStyle);
							...
							EditorGUILayout.LabelField(scaleRandMin.y.ToString("0.00") + " to " + scaleRandMax.y.ToString("0.00"),GUILayout.Width(76)); DoTipCheck("Y Axis Random Scaling Range Split Slider (Min/Max)");
							EditorGUILayout.MinMaxSlider(ref scaleRandMin.y,ref scaleRandMax.y,scaleMin,scaleMax); DoTipCheck("Y Axis Random Scaling Range Split Slider (Min/Max)");
	
							scaleRandMin.y = (float)System.Math.Round(scaleRandMin.y,2);
							scaleRandMax.y = (float)System.Math.Round(scaleRandMax.y,2);
							scaleRandMin.y = Mathf.Clamp(scaleRandMin.y,scaleMin,scaleMax);
                        ...
                        EditorGUILayout.BeginVertical(menuBlockStyle);
							...
							EditorGUILayout.LabelField(scaleRandMin.z.ToString("0.00") + " to " + scaleRandMax.z.ToString("0.00"),GUILayout.Width(76)); DoTipCheck("Z Axis Random Scaling Range Split Slider (Min/Max)");
							EditorGUILayout.MinMaxSlider(ref scaleRandMin.z,ref scaleRandMax.z,scaleMin,scaleMax); DoTipCheck("Z Axis Random Scaling Range Split Slider (Min/Max)");
	
							scaleRandMin.z = (float)System.Math.Round(scaleRandMin.z,2);
							scaleRandMax.z = (float)System.Math.Round(scaleRandMax.z,2);
							scaleRandMin.z = Mathf.Clamp(scaleRandMin.z,scaleMin,scaleMax);
...
//	EditorGUILayout.EndVertical();
	...
	EditorGUILayout.BeginHorizontal(brushStripStyle,GUILayout.Width(276)); //Begin Brush Slot Strip
	//	brushName = EditorGUILayout.TextField("Brush Name",brushName);  DoTipCheck("Name Your Brush");
	EditorGUILayout.EndHorizontal();
			...
			for(int i = 0; i < 6; i++)
			{
				EditorGUILayout.BeginVertical(brushSlotContainerStyle,GUILayout.Width(32)); //Begin Slot
					...
					EditorGUILayout.BeginHorizontal();	//Begin Slot Operations Duo
							...
							if(GUILayout.Button("",picButtonStyle, GUILayout.Width(16), GUILayout.Height(16)))
							{	
								SaveSettings(i,brushName);
								brushStateArray = GetSlotStates();
								brushNameArray = GetSlotNames();
							}DoTipCheck("Save Brush Settings to Slot" + (i + 1).ToString("00"));
							
						EditorGUI.BeginDisabledGroup(!brushStateArray[i]);
					...
					EditorGUI.BeginDisabledGroup(!brushStateArray[i]);
						...
						DoTipCheck("Brush Slot " + (i + 1).ToString("00") + " " + brushNameArray[i]);
...
//		bool xDown = false;
	...
	//6 : use settings to scale, rotate, and place the object
		...
		newObject.transform.position += (newObject.transform.right * window.positionOffset.x) + (upVector.normalized * window.positionOffset.y) + (newObject.transform.forward * window.positionOffset.z);
		Vector3 randomScale;
...
//		qb_ObjectContainer objectContainer = qb_ObjectContainer.GetInstance();
	...
	private static qb_RaycastResult DoPlacementRaycast(Vector3 castPosition,Vector3 rayDirection)
	{
		...
		if(hit.collider != null)
		{
			...
			if(window.paintToLayer == true)
			{
				...
				if( (1 << hit.collider.gameObject.layer & window.layerIndex) == 0)
					success = false;
			}
	...
	{	
		...
		if(placing)
			...
			rotationOffset = new Vector3(Random.Range(window.rotationRangeMin.x,window.rotationRangeMax.x),Random.Range(window.rotationRangeMin.y,window.rotationRangeMax.y),Random.Range(window.rotationRangeMin.z,window.rotationRangeMax.z));
...
//		window = null;
	...
	static void LoadDefaultSettings()
	{
		...
		#region Rotation Settings Vars
			...
			window.flipNormalAlign =		false;
		
			window.alignToStroke =			true;
			window.flipStrokeAlign =		false;

			//Offset
			//window.rotationRange = 			new Vector3(0f,0f,0f);

			window.rotationRangeMin =			new Vector3(0f,0f,0f);
			window.rotationRangeMax =			new Vector3(0f,0f,0f);
		#endregion
		
		#region Position Settings Vars
			window.positionOffset =				Vector3.zero;
		#endregion
...
////////SAVABLE BRUSHES
	...
	[SerializeField] bool[] brushStateArray = new bool[8];
	[SerializeField] string[] brushNameArray = new string[8]; 
	
	static void SaveSettings(int slotIndex, string brushNameString)
	{
			string prefix = "qbSettings_" + slotIndex.ToString() + "-";
		
			EditorPrefs.SetString(prefix + "BrushName", brushNameString);
		
		#region Brush Settings Vars
			EditorPrefs.SetFloat(prefix + "BrushRadius",window.brushRadius);
		
			EditorPrefs.SetFloat(prefix + "BrushRadiusMin",window.brushRadiusMin);
		
			EditorPrefs.SetFloat(prefix + "BrushRadiusMax",window.brushRadiusMax);
		
			EditorPrefs.SetFloat(prefix + "BrushSpacing",window.brushSpacing);
		
			EditorPrefs.SetFloat(prefix + "BrushSpacingMax",window.brushSpacingMin);
		
			EditorPrefs.SetFloat(prefix + "BrushSpacingMax",window.brushSpacingMax);

			EditorPrefs.SetFloat(prefix + "ScatterRadius",window.scatterRadius);
		#endregion
			
		#region Rotation Settings Vars
			//Alignment	
			EditorPrefs.SetBool(prefix + "AlignToNormal",window.alignToNormal);
			EditorPrefs.SetBool(prefix + "FlipNormalAlign",window.flipNormalAlign);
		
			EditorPrefs.SetBool(prefix + "AlignToStroke",window.alignToStroke);
			EditorPrefs.SetBool(prefix + "FlipStrokeAlign",window.flipStrokeAlign);
			
			//Offset
			//EditorPrefs.SetFloat(prefix + "RotationRangeX",window.rotationRange.x);
			//EditorPrefs.SetFloat(prefix + "RotationRangeY",window.rotationRange.y);
			//EditorPrefs.SetFloat(prefix + "RotationRangeZ",window.rotationRange.z);
		
			EditorPrefs.SetFloat(prefix + "RotationRangeMinX",window.rotationRangeMin.x);
			EditorPrefs.SetFloat(prefix + "RotationRangeMinY",window.rotationRangeMin.y);
			EditorPrefs.SetFloat(prefix + "RotationRangeMinZ",window.rotationRangeMin.z);
		
			EditorPrefs.SetFloat(prefix + "RotationRangeMaxX",window.rotationRangeMax.x);
			EditorPrefs.SetFloat(prefix + "RotationRangeMaxY",window.rotationRangeMax.y);
			EditorPrefs.SetFloat(prefix + "RotationRangeMaxZ",window.rotationRangeMax.z);
		
			//window.rotationRangeMin =			new Vector3(rotationRangeMinX,rotationRangeMinZ,rotationRangeMinZ);
			//window.rotationRangeMax =			new Vector3(rotationRangeMaxX,rotationRangeMaxY,rotationRangeMaxZ);
		#endregion
			
		#region Position Settings Vars
			EditorPrefs.SetFloat(prefix + "PositionOffsetX",window.positionOffset.x);
			EditorPrefs.SetFloat(prefix + "PositionOffsetY",window.positionOffset.y);
			EditorPrefs.SetFloat(prefix + "PositionOffsetZ",window.positionOffset.z);
		#endregion
			
		#region Scale Settings Vars	
			//The minimum and maximum possible scale
			EditorPrefs.SetFloat(prefix + "ScaleMin",window.scaleMin);
	
			EditorPrefs.SetFloat(prefix + "ScaleMax",window.scaleMax);
		
			//The minimum and maximum current scale range setting
			EditorPrefs.SetFloat(prefix + "ScaleRandMinX",window.scaleRandMin.x);
			EditorPrefs.SetFloat(prefix + "ScaleRandMinY",window.scaleRandMin.y);
			EditorPrefs.SetFloat(prefix + "ScaleRandMinZ",window.scaleRandMin.z);
		
			EditorPrefs.SetFloat(prefix + "ScaleRandMaxX",window.scaleRandMax.x);
			EditorPrefs.SetFloat(prefix + "ScaleRandMaxY",window.scaleRandMax.y);
			EditorPrefs.SetFloat(prefix + "ScaleRandMaxZ",window.scaleRandMax.z);
		
			EditorPrefs.SetFloat(prefix + "ScaleRandMinUniform",window.scaleRandMinUniform);
			
			EditorPrefs.SetFloat(prefix + "ScaleRandMaxUniform",window.scaleRandMaxUniform);

			EditorPrefs.SetBool(prefix + "ScaleUniform",window.scaleUniform);
		#endregion	
			
		#region Sorting Vars  
			//Selection
			EditorPrefs.SetBool(prefix + "PaintToSelection",window.paintToSelection);
			//Layers
			EditorPrefs.SetBool(prefix + "PaintToLayer",window.paintToLayer);
		
			EditorPrefs.SetInt(prefix + "LayerIndex",window.layerIndex);
		#endregion
	...
	static void LoadSettings(int slotIndex)
	{
			string prefix = "qbSettings_" + slotIndex.ToString() + "-";
		
			window.brushName =				EditorPrefs.GetString(prefix + "BrushName",window.brushName);
		#region Brush Settings Vars
			window.brushRadius	=			EditorPrefs.GetFloat(prefix + "BrushRadius",window.brushRadius);
			
			window.brushRadiusMin =			EditorPrefs.GetFloat(prefix + "BrushRadiusMin",window.brushRadiusMin);
		
			window.brushRadiusMax =			EditorPrefs.GetFloat(prefix + "BrushRadiusMax",window.brushRadiusMax);
		
			window.brushSpacing	=			EditorPrefs.GetFloat(prefix + "BrushSpacing",window.brushSpacing);
		
			window.brushSpacingMax =		EditorPrefs.GetFloat(prefix + "BrushSpacingMax",window.brushSpacingMax);
		
			window.brushSpacingMin =		EditorPrefs.GetFloat(prefix + "BrushSpacingMin",window.brushSpacingMin);
		
			window.scatterRadius =			EditorPrefs.GetFloat(prefix + "ScatterRadius",window.scatterRadius);
		#endregion
			
		#region Rotation Settings Vars
			//Alignment	
			window.alignToNormal =			EditorPrefs.GetBool(prefix + "AlignToNormal",window.alignToNormal);
			window.flipNormalAlign =		EditorPrefs.GetBool(prefix + "FlipNormalAlign",window.flipNormalAlign);
			
			window.alignToStroke =			EditorPrefs.GetBool(prefix + "AlignToStroke",window.alignToStroke);
			window.flipStrokeAlign =		EditorPrefs.GetBool(prefix + "FlipStrokeAlign",window.flipStrokeAlign);
		
			//Offset
			float rotationRangeMinX =			EditorPrefs.GetFloat(prefix + "RotationRangeMinX",window.rotationRangeMin.x);
			float rotationRangeMinY =			EditorPrefs.GetFloat(prefix + "RotationRangeMinY",window.rotationRangeMin.y);
			float rotationRangeMinZ =			EditorPrefs.GetFloat(prefix + "RotationRangeMinZ",window.rotationRangeMin.z);
		
			float rotationRangeMaxX =			EditorPrefs.GetFloat(prefix + "RotationRangeMaxX",window.rotationRangeMax.x);
			float rotationRangeMaxY =			EditorPrefs.GetFloat(prefix + "RotationRangeMaxY",window.rotationRangeMax.y);
			float rotationRangeMaxZ =			EditorPrefs.GetFloat(prefix + "RotationRangeMaxZ",window.rotationRangeMax.z);
		
			window.rotationRangeMin =			new Vector3(rotationRangeMinX,rotationRangeMinY,rotationRangeMinZ);
			window.rotationRangeMax =			new Vector3(rotationRangeMaxX,rotationRangeMaxY,rotationRangeMaxZ);
		...
		#region Position Settings Vars
			float positionOffsetX =				EditorPrefs.GetFloat(prefix + "PositionOffsetX",window.positionOffset.x);
			float positionOffsetY =				EditorPrefs.GetFloat(prefix + "PositionOffsetY",window.positionOffset.y);
			float positionOffsetZ =				EditorPrefs.GetFloat(prefix + "PositionOffsetZ",window.positionOffset.z);
		
			window.positionOffset =				new Vector3(positionOffsetX,positionOffsetY,positionOffsetZ);
		#endregion		
		
		#region Scale Settings Vars	
			//The minimum and maximum possible scale
			window.scaleMin =				EditorPrefs.GetFloat(prefix + "ScaleMin",window.scaleMin);
		
			window.scaleMax =				EditorPrefs.GetFloat(prefix + "ScaleMax",window.scaleMax);
		
			//The minimum and maximum current scale range setting
			float scaleRandMinX =			EditorPrefs.GetFloat(prefix + "ScaleRandMinX",window.scaleRandMin.x);
			float scaleRandMinY =			EditorPrefs.GetFloat(prefix + "ScaleRandMinY",window.scaleRandMin.y);
			float scaleRandMinZ =			EditorPrefs.GetFloat(prefix + "ScaleRandMinZ",window.scaleRandMin.z);
			window.scaleRandMin = 			new Vector3(scaleRandMinX,scaleRandMinY,scaleRandMinZ);
		
			float scaleRandMaxX =			EditorPrefs.GetFloat(prefix + "ScaleRandMaxX",window.scaleRandMax.x);
			float scaleRandMaxY =			EditorPrefs.GetFloat(prefix + "ScaleRandMaxY",window.scaleRandMax.y);
			float scaleRandMaxZ =			EditorPrefs.GetFloat(prefix + "ScaleRandMaxZ",window.scaleRandMax.z);
			window.scaleRandMax = 			new Vector3(scaleRandMaxX,scaleRandMaxY,scaleRandMaxZ);		
		
			window.scaleRandMaxUniform =	EditorPrefs.GetFloat(prefix + "ScaleRandMaxUniform",window.scaleRandMaxUniform);
		
			window.scaleRandMinUniform =	EditorPrefs.GetFloat(prefix + "ScaleRandMinUniform",window.scaleRandMinUniform);
		
			window.scaleUniform =			EditorPrefs.GetBool(prefix + "ScaleUniform",window.scaleUniform);
		#endregion	
			
		#region Sorting Vars  
			//Selection
			window.paintToSelection =		EditorPrefs.GetBool(prefix + "PaintToSelection",window.paintToSelection);
			//Layers
			window.paintToLayer =			EditorPrefs.GetBool(prefix + "PaintToLayer",window.paintToLayer);
		
			window.layerIndex =				EditorPrefs.GetInt(prefix + "LayerIndex",window.layerIndex);
		#endregion
		
		#region Repopulate the Prefab List
		string retreivedPrefabGroupString = EditorPrefs.GetString(prefix + "PrefabGUIDList",string.Empty);
		...
		window.prefabGroup = new qb_PrefabObject[0]; //clear out
	...
	static bool[] GetSlotStates()
	{
		
		bool[] stateArray = new bool[8];
		
		for(int i = 0; i < 6; i++)
		{
			stateArray[i] = GetSlotState(i);
		}
		
		return stateArray;
	}
	
	static bool GetSlotState(int slotIndex)
	{
		string prefix = "qbSettings_" + slotIndex.ToString() + "-";

		return EditorPrefs.HasKey(prefix + "BrushName");
	}
	...
	static string GetSlotName(int slotIndex)
	{
		...
		string brushName = EditorPrefs.GetString(prefix + "BrushName","Slot Empty");
	...
	static void ClearSlot(int slotIndex)
	{
		string prefix = "qbSettings_" + slotIndex.ToString() + "-";
			...
			EditorPrefs.DeleteKey(prefix + "BrushName");
		
		#region Brush Settings Vars
			EditorPrefs.DeleteKey(prefix + "BrushRadius");
		
			EditorPrefs.DeleteKey(prefix + "BrushRadiusMin");
		
			EditorPrefs.DeleteKey(prefix + "BrushRadiusMax");
		
			EditorPrefs.DeleteKey(prefix + "BrushSpacing");
		
			EditorPrefs.DeleteKey(prefix + "BrushSpacingMax");
		
			EditorPrefs.DeleteKey(prefix + "BrushSpacingMax");

			EditorPrefs.DeleteKey(prefix + "ScatterRadius");
		#endregion
			
		#region Rotation Settings Vars
			//Alignment	
			EditorPrefs.DeleteKey(prefix + "AlignToNormal");
			EditorPrefs.DeleteKey(prefix + "FlipNormalAlign");
			
			EditorPrefs.DeleteKey(prefix + "AlignToStroke");
			EditorPrefs.DeleteKey(prefix + "FlipStrokeAlign");
			//Offset
			EditorPrefs.DeleteKey(prefix + "RotationRangeX");
			EditorPrefs.DeleteKey(prefix + "RotationRangeY");
			EditorPrefs.DeleteKey(prefix + "RotationRangeZ");
		#endregion
		
		#region Position Settings Vars
			EditorPrefs.DeleteKey(prefix + "PositionOffsetX");
			EditorPrefs.DeleteKey(prefix + "PositionOffsetY");
			EditorPrefs.DeleteKey(prefix + "PositionOffsetZ");
		#endregion
		
		#region Scale Settings Vars	
			//The minimum and maximum possible scale
			EditorPrefs.DeleteKey(prefix + "ScaleMin");
	
			EditorPrefs.DeleteKey(prefix + "ScaleMax");
		
			//The minimum and maximum current scale range setting
			EditorPrefs.DeleteKey(prefix + "ScaleRandMinX");
			EditorPrefs.DeleteKey(prefix + "ScaleRandMinY");
			EditorPrefs.DeleteKey(prefix + "ScaleRandMinZ");
		
			EditorPrefs.DeleteKey(prefix + "ScaleRandMaxX");
			EditorPrefs.DeleteKey(prefix + "ScaleRandMaxY");
			EditorPrefs.DeleteKey(prefix + "ScaleRandMaxZ");
		
			EditorPrefs.DeleteKey(prefix + "ScaleRandMinUniform");
			
			EditorPrefs.DeleteKey(prefix + "ScaleRandMaxUniform");

			EditorPrefs.DeleteKey(prefix + "ScaleUniform");
		#endregion	
			
		#region Sorting Vars  
			//Selection
			EditorPrefs.DeleteKey(prefix + "PaintToSelection");
		
			//Layers
			EditorPrefs.DeleteKey(prefix + "PaintToLayer");
		
			EditorPrefs.DeleteKey(prefix + "LayerIndex");
		#endregion
		
			EditorPrefs.DeleteKey(prefix + "PrefabGUIDList");		
	}
...
//	[SerializeField] private GUIStyle scrollBarStyleH;
    ...
    private void SetStyleParameters()
	{
		...
		window.picButtonStyle.hover.textColor = Color.white;
		...
		//window.shortToggleStyle.
		//window.smallTextLabelStyle.alignment = TextAnchor.MiddleCenter;
		//window.smallTextLabelStyle.fontSize = 9;
	}
...
//		window.scrollBarStyleH = new GUIStyle(EditorGUIUtility.GetBuiltinSkin(EditorSkin.Inspector).horizontalScrollbar);
	...
	{
		//List<KeyValuePair<int,string>> layerList = new List<KeyValuePair<int, string>>();
		...
		for(int i = 0; i < 32; i++)
		{
			...
			layerArray[i] = name;
			//KeyValuePair<int,string> kvp = new KeyValuePair(i,32);
